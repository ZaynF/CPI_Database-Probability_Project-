...
namespace Probablity_Project_Txt
{
    public partial class Form7 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (choose_item6.SelectedItem.ToString() == "醫療費用")
            {
                StreamReader sr_mon = new StreamReader("Month.txt");
                StreamReader sr = new StreamReader("醫療費用.txt");
                for (int i = 0; i < 197; i++)
                {
            ...
            else if (choose_item6.SelectedItem.ToString() == "藥品及保健食品")
            {
                StreamReader sr_mon = new StreamReader("Month.txt");
                StreamReader sr = new StreamReader("藥品及保健食品.txt");
                for (int i = 0; i < 197; i++)
                {
                    month = sr_mon.ReadLine();
                    if (begin_mon.SelectedItem.ToString() == month) pos1 = i;
            ...
            else if (choose_item6.SelectedItem.ToString() == "醫療保健器材")
            {
                StreamReader sr_mon = new StreamReader("Month.txt");
                StreamReader sr = new StreamReader("醫療保健器材.txt");
                for (int i = 0; i < 197; i++)
                {
                    month = sr_mon.ReadLine();
                    if (begin_mon.SelectedItem.ToString() == month) pos1 = i;
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (choose_item6.SelectedItem.ToString() == "醫療費用")
            {
                try
                {
                    ...
                    System.Diagnostics.Process.Start("醫療費用.html");
                }
                catch (Exception ex)
                {
            ...
            else if (choose_item6.SelectedItem.ToString() == "藥品及保健食品")
            {
                ...
                {
                    linkLabel1.LinkVisited = true;
                    System.Diagnostics.Process.Start("藥品及保健食品.html");
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Unable to open link that was clicked.");
            ...
            else if (choose_item6.SelectedItem.ToString() == "醫療保健器材")
            {
                ...
                {
                    linkLabel1.LinkVisited = true;
                    System.Diagnostics.Process.Start("醫療保健器材.html");
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Unable to open link that was clicked.");
